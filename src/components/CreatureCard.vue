...
<template>
  <div
    class="w-full bg-slate-800 rounded-2xl shadow-black shadow-xl border border-slate-700 overflow-hidden flex flex-col hover:border-blue-500 transition-colors duration-300"
  >
    <div
      class="relative h-24 bg-linear-to-b from-slate-700 to-slate-900 p-2 flex items-center justify-center group bg-cover bg-center"
    ...
    <div class="p-2 flex flex-col grow text-center">
      ...
      <div
        v-if="creature.expand?.wezenknal_relatie"
        class="mb-2 flex flex-col items-center border-t border-slate-700/50"
      >
      ...
      <div class="mt-auto pt-4 border-t border-slate-700/50">
        ...
        <div class="flex justify-center items-center gap-3 min-h-8">
          <img
            v-if="creature.expand?.reeks3a_relatie"
            :src="getIconUrl(creature.expand.reeks3a_relatie)"
            class="w-8 h-8 object-contain"
            title="Powerup A"
          />
          <img
            v-if="creature.expand?.reeks3b_relatie"
            :src="getIconUrl(creature.expand.reeks3b_relatie)"
            class="w-8 h-8 object-contain"
            title="Powerup B"
          />
          <img
            v-if="creature.expand?.reeks3c_relatie"
            :src="getIconUrl(creature.expand.reeks3c_relatie)"
            class="w-8 h-8 object-contain"
            title="Powerup C"
          />
          <span
            v-if="!creature.expand?.reeks3a_relatie && !creature.expand?.reeks3b_relatie"
